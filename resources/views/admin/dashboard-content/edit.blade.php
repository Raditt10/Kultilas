...
@section('title', 'Edit Konten Dashboard - Admin Panel')
...
<div>
        ...
        <div class="flex items-center mb-8">
            ...
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-300">
                    Edit konten "{{ $dashboardContent->title }}"
                </p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <form action="{{ route('admin.dashboard-content.update', $dashboardContent) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        @foreach($types as $key => $type)
                            <label class="cursor-pointer">
                                <input type="radio" name="type" value="{{ $key }}" 
                                       ...
                                       {{ old('type', $dashboardContent->type) == $key ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" id="category" 
                               value="{{ old('category', $dashboardContent->category) }}"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white"
                    ...
                    <div>
                        ...
                        <input type="text" name="author" id="author" 
                               value="{{ old('author', $dashboardContent->author) }}"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white"
                    ...
                    <div>
                        ...
                        <div class="flex">
                            <input type="text" name="icon" id="icon" 
                                   value="{{ old('icon', $dashboardContent->icon) }}"
                                   class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-l-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white"
                                   placeholder="Misal: fas fa-star, fas fa-medal">
                            <div class="px-3 py-2 bg-gray-100 dark:bg-gray-600 border border-l-0 border-gray-300 dark:border-gray-600 rounded-r-lg">
                                <i class="{{ old('icon', $dashboardContent->icon) ?: 'fas fa-question' }} text-gray-600 dark:text-gray-300" id="icon-preview"></i>
                            </div>
                        </div>
                        @error('icon')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" id="order" 
                               value="{{ old('order', $dashboardContent->order) }}" min="0"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white"
                               placeholder="0">
                        @error('order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="content" id="content" rows="6" 
                              class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white"
                              placeholder="Tulis konten di sini..." required>{{ old('content', $dashboardContent->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="expires_at" id="expires_at" 
                               value="{{ old('expires_at', $dashboardContent->expires_at ? $dashboardContent->expires_at->format('Y-m-d\TH:i') : '') }}"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white">
                    ...
                    <div>
                        ...
                        <label class="flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" 
                                   {{ old('is_active', $dashboardContent->is_active) ? 'checked' : '' }}
                ...
                <!-- Status Info -->
                @if($dashboardContent->is_expired)
                    <div class="mt-4 p-4 bg-red-100 border border-red-300 rounded-lg">
                        <div class="flex items-center">
                            <i class="fas fa-exclamation-triangle text-red-600 mr-2"></i>
                            <p class="text-red-800">
                                <strong>Konten Expired:</strong> Konten ini sudah kadaluarsa pada {{ $dashboardContent->expires_at->format('d M Y H:i') }}
                            </p>
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-8 flex flex-col sm:flex-row gap-4">
                    ...
                    <a href="{{ route('admin.dashboard-content.show', $dashboardContent) }}" 
                       class="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white px-8 py-3 rounded-lg font-semibold transition-all text-center">
...
document.getElementById('icon').addEventListener('input', function() {
    ...
    iconPreview.className = (this.value || 'fas fa-question') + ' text-gray-600 dark:text-gray-300';
});
...
document.querySelectorAll('.type-radio').forEach(radio => {
    radio.addEventListener('change', function() {
        ...
        if (iconMap[this.value]) {
            ...
            document.getElementById('icon-preview').className = iconMap[this.value] + ' text-gray-600 dark:text-gray-300';
        }
    });
});
</script>
