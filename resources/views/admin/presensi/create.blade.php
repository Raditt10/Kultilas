            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                        <i class="fas fa-plus-circle text-green-600 mr-3"></i>Tambah Presensi
                    </h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-300">Tambahkan data presensi siswa ekstrakurikuler</p>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="id_eskul" id="id_eskul" required
                                    ...
                                    @foreach(\App\Models\Eskul::all() as $eskul)
                                        <option value="{{ $eskul->id_eskul }}" {{ old('id_eskul') == $eskul->id_eskul ? 'selected' : '' }}>
                                            {{ $eskul->nama_eskul }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="status_hadir" id="status_hadir" required
                                    ...
                                    <option value="alpa" {{ old('status_hadir') == 'alpa' ? 'selected' : '' }}>Alpa</option>
                                </select>
                                @error('status_hadir')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors duration-200">
                                    <option value="tunda" {{ old('status') == 'tunda' ? 'selected' : '' }}>Tertunda</option>
                            ...
                            <div>
                                ...
                                <textarea name="catatan" id="catatan" rows="3" placeholder="Tambahkan catatan (opsional)"
                                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors duration-200 resize-none">{{ old('catatan') }}</textarea>
                                @error('catatan')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
